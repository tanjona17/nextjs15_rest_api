...
export const GET = async (request: Request, paramId: { params: any }) => {
  const blogId = paramId.params.blog;
  ...
  try {
    ...
    if (!blogId || !Types.ObjectId.isValid(blogId)) {
      return new NextResponse(
        JSON.stringify({ message: "Ivalid or missing blogId" }),
        { status: 400 }
      );
    }
    ...
    return new NextResponse("Error while getting blog", { status: 500 });
  }
};
...
export const PATCH = async (request: Request, paramId: { params: any }) => {
  ...
  try {
    ...
    return new NextResponse(
      JSON.stringify({ message: "Blog updated successfully", updatedBlog })
    );
  } catch (error: any) {
